# Oh-My-Zsh update delete symlink
...
# 	div=$(python -c 'import sys;res=sys.argv[1:];print(f"{int(res[1])/int(res[0]):.4f}");' "$width" "$height")
# 	if [[ "$div" == "0.5625" ]]; then

	div=$(python -c 'import sys,math;res=sys.argv[1:];print(f"{int(res[1])/int(res[0]):.4f}");' "$width" "$height")

	PERFECT_DIV=0.5625
	MAX_DELTA=0.0002

	MIN=$((PERFECT_DIV-MAX_DELTA))
	MAX=$((PERFECT_DIV+MAX_DELTA))

# 	echo $MIN $MAX

	if [[ $div -gt MIN && $div -lt MAX ]]; then
		printf "\u001B[32m[ IS ]"
	else
		printf "\u001B[31m[ NO ]"
	fi
